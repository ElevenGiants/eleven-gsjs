...
var label = "Heli Kitty";
var version = "1355090246";
var name_single = "Heli Kitty";
var name_plural = "Heli Kitties";
var article = "a";
var description = "This kitty never needs to worry about landing on its feet.";
var is_hidden = false;
var has_info = true;
...
var base_cost = 0;
var input_for = [];
var parent_classes = ["npc_kitty_chicken", "npc_chicken", "npc_walkable", "npc"];
...
verbs.butter_up = { // defined by npc_kitty_chicken
	"name"				: "butter up",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
...
verbs.pet = { // defined by npc_kitty_chicken
	"name"				: "pet",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Pet the kitty",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){

		if (this.isRooked() || this.isSad() || this['!is_flying']) {
			return {state:null};
		}
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		this.stopMoving();
		this.setAndBroadcastState("3sleepStart");
		this.apiSetTimer("setAndBroadcastState", 1000* (26/30), "3sleep");  

		var time = 1000* ((26/30) + 1*(58/30)) // 1 sleep cycles + intro 
		this.apiSetTimer("endSleep", time + 10);  

		pc.announce_sound("FLYING_KITTY_PURR");
		...
		this.sendBubble("purrrrrr", 2*1000, pc, slugs);

		var pre_msg = this.buildVerbMessage(msg.count, 'pet', 'petted', failed, self_msgs, self_effects, they_effects);
		if (!suppress_activity && pre_msg) pc.sendActivity(pre_msg);

		if (!this.petted_for_feat) this.petted_for_feat = {};
		if (!this.petted_for_feat[pc.tsid]) this.petted_for_feat[pc.tsid] = 0;
		if (this.petted_for_feat[pc.tsid] < 3) {
			pc.feats_increment('animal_love', 1);
			this.petted_for_feat[pc.tsid]++;
		}
...
verbs.rename = { // defined by npc_kitty_chicken
	"name"				: "rename",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Change this kitty's name",
	"is_drop_target"		: false,
	"handler"			: function(pc, msg, suppress_activity){
...
verbs.name = { // defined by npc_kitty_chicken
	"name"				: "name",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Give this kitty a name",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
...
verbs.set_free = { // defined by npc_kitty_chicken
	"name"				: "set free",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"get_tooltip"			: function(pc, verb, effects){
		...
		if(this.user_name) {
			var result = "Release a kitty named "+this.user_name+" into the wilderness. "+verb.tooltip;
		} else {
			var result = "Release this kitty into the wilderness. "+verb.tooltip;
		}
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		this.free_pc = pc;
		if(this.user_name) {
			var prompt_text = "Are you sure you want to let your kitty named "+this.user_name+" out into the wilderness?";
		} else {
			var prompt_text = "Are you sure you want to let your kitty out into the wilderness?";
		}
...
verbs.revive = { // defined by npc_kitty_chicken
	"name"				: "revive",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Help this kitty escape the effects of The Rook. Costs $energy_cost energy",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
...
verbs.talk_to = { // defined by npc_kitty_chicken
	"name"				: "talk to",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
...
verbs.apply_youth = { // defined by npc_kitty_chicken
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		var pre_msg = this.buildVerbMessage(msg.count, 'apply youth potion', 'applied youth potion', failed, self_msgs, self_effects, they_effects);
		if (!suppress_activity && pre_msg) pc.sendActivity(pre_msg);
...
verbs.incubate = { // defined by npc_kitty_chicken
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		var pre_msg = this.buildVerbMessage(msg.count, 'request incubation', 'requested incubation', failed, self_msgs, self_effects, they_effects);
		if (!suppress_activity && pre_msg) pc.sendActivity(pre_msg);
...
verbs.apply_balm = { // defined by npc_kitty_chicken
	"name"				: "apply balm to",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Revive this kitty with some Rook Balm",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
...
verbs.squeeze = { // defined by npc_kitty_chicken
	"name"				: "squeeze",
	"ok_states"			: ["in_location"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		var pre_msg = this.buildVerbMessage(msg.count, 'squeeze', 'squeezed', failed, self_msgs, self_effects, they_effects);
		if (!suppress_activity && pre_msg) pc.sendActivity(pre_msg);
...
function parent_verb_npc_chicken_butter_up(pc, msg, suppress_activity){
	var failed = 0;
	var orig_count = this.count;
	var self_msgs = [];
	var self_effects = [];
...
function parent_verb_npc_chicken_butter_up_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_apply_youth(pc, msg, suppress_activity){
...
function parent_verb_npc_chicken_apply_youth_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_apply_balm(pc, msg, suppress_activity){
	function is_balm(it){ return it.class_tsid == 'potion_rook_balm' ? true : false; }
	var balm = pc.findFirst(is_balm);
...
function parent_verb_npc_chicken_apply_balm_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_talk_to(pc, msg, suppress_activity){
	var failed = 1;
	var orig_count = this.count;
	var self_msgs = [];
	var self_effects = [];
...
function parent_verb_npc_chicken_talk_to_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_set_free(pc, msg, suppress_activity){
	this.free_pc = pc;
	if(this.user_name) {
...
function parent_verb_npc_chicken_set_free_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_rename(pc, msg, suppress_activity){
	var args = {
		input_label: 'My new name:',
		cancelable: true,
		input_focus: true
...
function parent_verb_npc_chicken_rename_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_name(pc, msg, suppress_activity){
	var args = {
		input_label: 'My new name:',
		cancelable: true,
		input_focus: true
...
function parent_verb_npc_chicken_name_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_chicken_revive(pc, msg, suppress_activity){
	var failed = 0;
	var orig_count = this.count;
	var self_msgs = [];
	var self_effects = [];
...
function parent_verb_npc_chicken_revive_effects(pc){
	return this.getReviveEffects(pc);
};

function parent_verb_npc_chicken_squeeze(pc, msg, suppress_activity){
	this.stopMoving();
...
function parent_verb_npc_chicken_squeeze_effects(pc){
	return pc.trySkillPackage('ak_chicken_squeeze');
};

function parent_verb_npc_chicken_incubate(pc, msg, suppress_activity){
	//
...
function parent_verb_npc_chicken_incubate_effects(pc){
	// no effects code in this parent
};

function endSleep(){ // defined by npc_kitty_chicken
	this.state = "3fly";
...
function fly(){ // defined by npc_kitty_chicken
...
function freeChicken(){ // defined by npc_kitty_chicken
	if(this.user_name) {
		this.free_pc.sendActivity("The kitty named "+this.user_name+" has gone to enjoy its life in the wilderness.");
	} else {
		this.free_pc.sendActivity("Your kitty has gone to enjoy its life in the wilderness.");
	}

	this.state = "3disappear";
	this.broadcastState();

	this.apiSetTimer("apiDelete", 2000);
}

function leave(){ // defined by npc_kitty_chicken
	this.state = "3disappear";
	this.apiSetTimer("apiDelete", 1500);
}

function onCreate(){ // defined by npc_kitty_chicken
	...
	this.apiSetTimer("startMoving", (46/30) * 1000 + 500);
	this.apiSetTimer('onPause', randInt(3000, 11000));

	this.broadcastStatus();
}

function onInTheAir(){ // defined by npc_kitty_chicken
	this.state = "3fly";
}

function onLadderLanding(){ // defined by npc_kitty_chicken
	this.state = "3fly";
	this['!is_flying'] = true;
}

function onLadderTakeoff(){ // defined by npc_kitty_chicken
	this.state = '3fly';
	this['!is_flying'] = true;
}

function onLanding(){ // defined by npc_kitty_chicken
	this.state = "3fly";

	this.broadcastState();
}

function onPathing(args){ // defined by npc_kitty_chicken
...
function onPause(){ // defined by npc_kitty_chicken
	if (this.container.getNumActivePlayers()){
		if (!this['!is_flying']  && !this.isRooked() && !this.is_sad && this.state != "3sleep" && this.state != "3sleepStart"){
			this.apiStopMoving();
			var choice = choose_one(this.kitty_pause_state_choices);
			this.state = choice[1];
...
function onPlatformLanding(){ // defined by npc_kitty_chicken
	this['!is_flying'] = false;
...
function onPlayerCollision(pc){ // defined by npc_kitty_chicken
	...
	if (this.state != "3sleep" && this.state != "3sleepStart" && is_chance(chance)){
		this.fly();
		pc.announce_sound("FLYING_KITTY_MEW");
		this.sendBubble('Mew!', 1*1000);
	}

	apiResetThreadCPUClock("onPlayerCollision_"+this.class_tsid);
}

function onPlayerEnter(pc){ // defined by npc_kitty_chicken
	this.npc_onPlayerEnter(pc);

	if (!this.apiTimerExists("onPause")) {
		this.apiSetTimer('onPause', randInt(3000, 11000));
	}
}

function onPlayerExit(pc){ // defined by npc_kitty_chicken
	this.npc_onPlayerExit(pc);
}

function onRookedInterval(){ // defined by npc_kitty_chicken
	if (!this.isRooked()){
		return;
	}
...
function onStatus(pc){ // defined by npc_kitty_chicken
...
function onWaitStart(){ // defined by npc_kitty_chicken
	if (this.isRooked()) return;
	if (this.isSad()) return;
...
function setMovementLimits(x_pos, y_pos, width){ // defined by npc_kitty_chicken
	// kittys can't be confined

	return;
}

function startMoving(){ // defined by npc_kitty_chicken
	if (this.state != "3sleep" && this.state != "3sleepStart") {
		this.startMovingLimited();
	}
	else { 
		log.info("KITTY still sleeping");
	}
}

function stopMoving(){ // defined by npc_kitty_chicken
	this.apiStopMoving();
	var idle_state = '3fly';
...
// global block from npc_kitty_chicken
// How long and what state to pause in. Chosen randomly.
var kitty_pause_state_choices = [
	[5.0/30 + 1, '3blink'],
	[5.0/30 + 1, '3blink'],
	[5.0/30 + 1, '3blink'],
	[38.0/30, "3happy"],
...
function npc_walkable_onStuck(){ // defined by npc_walkable
	//log.info('STUCK! '+this);
	this.turnAround();
	this.apiSetTimer('startMoving', 1000);
}
...
function npc_clearMovementLimits(){ // defined by npc
	delete this.move_limits;
}
...
function parent_fly(){ // defined by npc_chicken
	if (this.isRooked()) return;
	if (this.isSad()) return;
	if (this.isWaiting) return;
...
function parent_freeChicken(){ // defined by npc_chicken
	if(this.user_name) {
...
function parent_leave(){ // defined by npc_chicken
	this.apiDelete();
}

function parent_onCreate(){ // defined by npc_chicken
...
function parent_onInTheAir(){ // defined by npc_chicken
...
function parent_onLadderLanding(){ // defined by npc_chicken
...
function parent_onLadderTakeoff(){ // defined by npc_chicken
...
function parent_onLanding(){ // defined by npc_chicken
	this.checkChickenSticks();
...
function parent_onPathing(args){ // defined by npc_chicken
	//log.info('pathing callback: '+args.status);
	if (args.status == 3 || args.status == 4){
		if (this.classProps.walk_type == 'roam' || this.classProps.walk_type == 'pace'){
			this.stopMoving();
...
function parent_onPause(){ // defined by npc_chicken
...
function parent_onPlatformLanding(){ // defined by npc_chicken
	this['!is_flying'] = false;
...
function parent_onPlayerCollision(pc){ // defined by npc_chicken
	apiResetThreadCPUClock();
	if (this.getInstanceProp('is_announcement') == 1) return;
	if (this.isRooked()) {
		this.rookedCollision(pc);
...
function parent_onPlayerEnter(pc){ // defined by npc_chicken
...
function parent_onPlayerExit(pc){ // defined by npc_chicken
...
function parent_onRookedInterval(){ // defined by npc_chicken
	if (!this.isRooked()){
		return;
	}
...
function parent_onStatus(pc){ // defined by npc_chicken
	var is_rook_verbs = this.isRooked();
	var is_tend_verbs = !is_rook_verbs;
...
function parent_onWaitStart(){ // defined by npc_chicken
	if (this.isRooked()) return;
	if (this.isSad()) return;
...
function parent_setMovementLimits(x_pos, y_pos, width){ // defined by npc_chicken
...
function parent_startMoving(){ // defined by npc_chicken
...
var responses = {
	"rooked": [
		"Ouch! My tail! HELP ME!",
		"I've been rooked. What's the opposite of purring?",
		"HISS! Scratch! Nasty rook!",
		"Please help me.",
	],
...
var itemDef = {
	...
	asset_swf_v	: "\/c2.glitch.bz\/items\/2012-11\/npc_kitty_chicken-1353030899.swf",
	admin_props	: true,
	obey_physics	: false,
	in_background	: false,
	in_foreground	: true,
...
log.info("npc_kitty_chicken.js LOADED");
