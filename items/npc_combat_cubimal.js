...
var name_single = "ComCu";
var name_plural = "";
var article = "a";
var description = "testing cubimal combat idea";
var is_hidden = false;
var has_info = true;
...
var base_cost = 0;
var input_for = [];
var parent_classes = ["npc_combat_cubimal", "npc_cubimal_base", "npc"];
...
function initInstanceProps(){
	...
	this.instanceProps.level = "1";	// defined by npc_combat_cubimal
	this.instanceProps.xp = "0";	// defined by npc_combat_cubimal
	this.instanceProps.next_level_amount = "10";	// defined by npc_combat_cubimal
	this.instanceProps.health = "15";	// defined by npc_combat_cubimal
}
...
verbs.paper_battle = { // defined by npc_combat_cubimal
	"name"				: "Panache (battle)",
	"ok_states"			: ["in_pack"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
...
verbs.scissors_battle = { // defined by npc_combat_cubimal
	"name"				: "Wile (battle)",
	"ok_states"			: ["in_pack"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
...
verbs.battle = { // defined by npc_combat_cubimal
...
function parent_verb_npc_cubimal_base_battle(pc, msg, suppress_activity){
	var failed = 0;
	var orig_count = this.count;
	var self_msgs = [];
	var self_effects = [];
...
function parent_verb_npc_cubimal_base_battle_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_cubimal_base_paper_battle(pc, msg, suppress_activity){
	var failed = 0;
	var orig_count = this.count;
	var self_msgs = [];
	var self_effects = [];
...
function parent_verb_npc_cubimal_base_paper_battle_effects(pc){
	// no effects code in this parent
};

function parent_verb_npc_cubimal_base_scissors_battle(pc, msg, suppress_activity){
	var failed = 0;
	var orig_count = this.count;
	var self_msgs = [];
	var self_effects = [];
...
function parent_verb_npc_cubimal_base_scissors_battle_effects(pc){
	// no effects code in this parent
};

function checkWin(targetC){ // defined by npc_combat_cubimal
	if(targetC.H<=0) //if dead 
	{
	    this.battleOver = true;                                       //end battle
	    //targetC.sendBubble("LOSER", 4000, targetC.fighting_owner);
	                var annc = {
	...
	else //if not dead, output remaining health and say insult
	{
	    ...
	    this.container.sendActivity("CUBI HEALTH: "+targetC.H);
	}
}

function getLabel(){ // defined by npc_combat_cubimal
	if (this.user_name) 
	{  
...
function getType(){ // defined by npc_combat_cubimal
...
function makeWinner(pc,targetC){ // defined by npc_combat_cubimal
	...
	if(xp + xpEarned >= nla) //if leveled up
	{
	   ...
	   this.sendBubble("LEVEL "+this.getInstanceProp('level')+"!", 4000, pc);
	   this.name_single = this.getLabel();
	}
	else
	{
	   this.setInstanceProp('xp', xp+xpEarned); //increase xp amount
	   this.sendBubble("+"+xpEarned+"xp!", 4000, pc);
	}
...
function onBattleLoop(targetC,pc){ // defined by npc_combat_cubimal
	...
	log.info("CUBI: this.start_point is "+this.start_point);
	this.apiFindPath(this.start_point, this.y, 0, 'onPathing');   //move back to start position
...
function onBattleStart(pc,targetC){ // defined by npc_combat_cubimal
...

	{
	    ...
	    var damage = Math.floor((Math.random()*(level+4))+(level)+mod); //calc damage/insult(max)(min)
	...
	if(this.targetC)
	{
	    ...
	    if(this.hasMidPoint==false)
	    {
	         if(this.targetC)
		 {
	  	     ...
	  	     {
	  	         this.start_point = this.x-(dis/2)+50; //set start point
	   	     }
	   	     else
	   	     {
	    	         this.start_point = this.x+(dis/2)-50; //set start point
...
function onCreate(){ // defined by npc_combat_cubimal
...
function onPathing(args){ // defined by npc_combat_cubimal
	//this.fsm_event_notify('pathing', args); 

	if(this.is_fighting==false)
	{
...
function search(pc){ // defined by npc_combat_cubimal
	if (!this.targetC)
	{
	    var container = this.container;
	    if(!container.find_items) return;
	    var cubi = container.find_items('npc_combat_cubimal'); 
	...
	{
	    this.sendBubble("Come at me, Bro!", 3000, this.fighting_owner);
	}
	else
	{
...
function showInsult(amount,pc,targetC){ // defined by npc_combat_cubimal
	if((amount == this.targetC.speech) && (amount != "*choke*") && (amount != null))
	{
	    amount = "No, "+amount;
	}
	return this.sendBubble(amount, 920, pc);
}

function taunt(targetC,pc){ // defined by npc_combat_cubimal
...
function parent_checkWin(targetC){ // defined by npc_cubimal_base
...
function parent_getLabel(){ // defined by npc_cubimal_base
	if (this.user_name) 
	{  
	     return this.user_name+" Lvl."+this.getInstanceProp('level');
	}
...
function parent_getType(){ // defined by npc_cubimal_base
	if(this.type == "r" && this.targetC.type == "s")
	{
	   var mod = 1;
	}
...
function parent_makeWinner(pc,targetC){ // defined by npc_cubimal_base
	var xpEarned = intval(targetC.getInstanceProp('level')*10);
	var nla = intval(this.getInstanceProp('next_level_amount'));
	var xp = intval(this.getInstanceProp('xp'));
...
function parent_onBattleLoop(targetC,pc){ // defined by npc_cubimal_base
	//this.container.sendActivity(this+"State: LOOP");
...
function parent_onBattleStart(pc,targetC){ // defined by npc_cubimal_base
	//this.container.sendActivity(this+ " State: START");
...
function parent_onCreate(){ // defined by npc_cubimal_base
	this.setAndBroadcastState('1');
...
function parent_onPathing(args){ // defined by npc_cubimal_base
	//this.fsm_event_notify('pathing', args); 
...
function parent_search(pc){ // defined by npc_cubimal_base
	if (!this.targetC)
	{
	    var container = this.container;
	    if(!container.find_items) return;
...
function parent_showInsult(amount,pc,targetC){ // defined by npc_cubimal_base
	if((amount == this.targetC.speech) && (amount != "*choke*") && (amount != null))
	{
	    amount = "No, "+amount;
	}
...
function parent_taunt(targetC,pc){ // defined by npc_cubimal_base
	this.targetC.onBattleStart(pc,this); //call battle on opponent
}
...
log.info("npc_combat_cubimal.js LOADED");
