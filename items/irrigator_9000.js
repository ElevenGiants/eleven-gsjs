...
var label = "Irrigator 9000";
var version = "1355086256";
var name_single = "Irrigator 9000";
var name_plural = "Irrigator 9000s";
var article = "an";
var description = "Galvanised for your pleasure, the Irrigator 9000 offers unparalleled waterability for the constant gardener. With reinforced spout for a five-times longer life - you get what you pay for.";
var is_hidden = false;
var has_info = true;
...
var base_cost = 750;
var input_for = [];
var parent_classes = ["irrigator_9000", "watering_can", "tool_base", "takeable"];
...
var classProps = {
	...
	"points_capacity"	: "500",	// defined by tool_base (overridden by irrigator_9000)
...
function initInstanceProps(){
	this.instanceProps = {};
	this.instanceProps.points_remaining = "500";	// defined by tool_base (overridden by irrigator_9000)
...
verbs.soak = { // defined by watering_can
	"name"				: "soak",
	"ok_states"			: ["in_pack"],
	"requires_target_pc"		: true,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 57,
	"tooltip"			: "Soak others with water",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		if (msg.pc_tsid == pc.tsid){
			failed = 1;
			self_msgs.push("You can't soak yourself!");
		}
		else{
			...
			if (!target){
				failed = 1;
				self_msgs.push("Huh. They must have run away.");
			}
			else{
				target.metabolics_lose_mood(3);
				target.metabolics_add_energy(5);

				target.sendActivity(pc.label+" dumps water all over you. You are soaked!");

				self_msgs.push("Haha!");

				if (pc.getQuestStatus('watering_time') == 'todo'){
					if (!pc.soaked){ pc.soaked = {}; }

					if (!pc.soaked[target.tsid]){
						pc.soaked[target.tsid] = 1;
						pc.quests_inc_counter('players_soaked', 1);
					}
				}
				else if (pc.soaked){
					delete pc.soaked;
		...
		var pre_msg = this.buildVerbMessage(msg.count, 'soak', 'soaked', failed, self_msgs, self_effects, they_effects, target);
...
verbs.water = { // defined by watering_can
	"name"				: "water",
	"ok_states"			: ["in_pack"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 58,
	"tooltip"			: "Or, drag can to a plant",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		// hmm - there are multiple things which could be watered....

		var result = this.proxyVerb(pc, msg, function(it){ return ((it.is_trant || it.class_tsid=='wood_tree') && !it.is_poisoned) ? true : false }, 100, 'water');

		if (result) { return true; }
		else if (!this.isWorking(2)) { 
			// Slightly hacky fix for case where watering doesn't work because the can doesn't have enough 
			// wear points left. This check might need to be changed if we ever change the amount of wear 
			// done to a watering can by a player with no LGT skills.
			return false; 
		}
		else {
			pc.sendActivity("But there's nothing to water!");
		}
...
function getHitPointModifier(pc){ // defined by watering_can
	if (pc.imagination_has_upgrade('lightgreenthumb_tool_wear')) return 0.5;
...
function tool_base_getHitPointModifier(pc){ // defined by tool_base
	return 1;
}
...
function getDescExtras(pc){
	var out = [];
	if (pc && (!pc.skills_has("light_green_thumb_1"))) out.push([2, "You can use this, but it's much better to learn <a href=\"\/skills\/6\/\" glitch=\"skill|light_green_thumb_1\">Light Green Thumb<\/a> first."]);
	...
	if (pc && !(pc.skills_has("light_green_thumb_3") && pc.skills_has("tinkering_5"))) out.push([2, "The recipe for this will become available after you learn <a href=\"\/skills\/8\/\" glitch=\"skill|light_green_thumb_3\">Light Green Thumb III<\/a> and <a href=\"\/skills\/76\/\" glitch=\"skill|tinkering_5\">Tinkering V<\/a>."]);
	return out;
}
...
function getAssetInfo(){
	return {
		'position': {"x":-28,"y":-44,"w":54,"h":43},
		'thumb': "",
	};
};
...
var itemDef = {
	...
	asset_swf_v	: "\/c2.glitch.bz\/items\/2012-05\/irrigator_9000-1338243120.swf",
	admin_props	: true,
	obey_physics	: true,
	in_background	: false,
	in_foreground	: false,
