...
var name_single = "Crafty Tasker";
var name_plural = "Crafty Taskers";
var article = "a";
...
var base_cost = 0;
var input_for = [];
var parent_classes = ["crafty_tasker", "npc"];
...
function initInstanceProps(){
	...
	this.instanceProps.max_step_count = "2";	// defined by crafty_tasker
	this.instanceProps.time_multiplier = "1";	// defined by crafty_tasker
}
...
verbs.craft = { // defined by crafty_tasker
	"name"				: "craft",
	"ok_states"			: ["in_pack"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		var args = {
			input_label: 'What would you like to craft?',
			cancelable: true,
			input_focus: true,
			input_max_chars: 150,
...
function craftItem(class_tsid, count){ // defined by crafty_tasker
	if (!this.make_spec) return {'ok': 0, 'error': 'Make spec was not set.'};

	var pc = this.container.owner;
	if (!pc) return {'ok': 0, 'error': 'Owner could not be determined.'};

	this.onStep(pc, msg, this.make_spec, 0);
}

function onConversation(pc, msg){ // defined by crafty_tasker
	if (msg.choice == 'make'){
		this.failed = false;
		this.onStep(pc, msg, this.make_spec, 0);
		return this.conversation_end(pc, msg);
...
function onEndToolUse(details){ // defined by crafty_tasker
	details.pc.announce_sound_stop(details.tool_class_tsid.toUpperCase(), 0);
}

function onExecuteRecipe(details){ // defined by crafty_tasker
...
function onFail(pc){ // defined by crafty_tasker
	this.failed = true;
	pc.sendActivity('Auto-make failed because one of the ingredients could not be created');
}

function onInputBoxResponse(pc, uid, value){ // defined by crafty_tasker
	if (uid == 'make' && value){

		var values = value.split(' ');
		if (!values[1]) values[1] = 1;

		var full_spec = 	utils.craftytasking_get_build_spec(pc, values[0], values[1], true);

		if (full_spec && this.getInstanceProp('max_step_count') > 0){
			this.make_spec = utils.craftytasking_limited_spec(full_spec, this.getInstanceProp('max_step_count'));
		}else{
			this.make_spec = full_spec;
		}

		log.info('make_spec: '+this.make_spec);

		if (!this.make_spec || !this.make_spec['ingredients']) {
...
function onOverlayDismissed(pc, payload){ // defined by crafty_tasker
	pc.announce_sound_stop(payload.tool_class_tsid.toUpperCase());
}

function onStep(pc, msg, spec, duration){ // defined by crafty_tasker
	...
	if (spec['elements'] && spec['elements']['ingredients']){
		for (var i in spec['elements']['ingredients']){
			this.events_add({	callback: 'onToolUse', 
							...
							duration: (spec['elements']['ingredients'][i]['wait_ms'] * this.getInstanceProp('time_multiplier')), 
							tool_verb:'crush', 
	...
	if (spec['tool']){
		this.events_add({	callback: 'onToolUse', 
						...
						count: spec['count'], 
						duration: (spec['tool_ms'] * this.getInstanceProp('time_multiplier')), 
						tool_verb:spec['tool_verb'], 
						ingredient_class:ingredient_class, 
						ingredient_count:ingredient_count}, 
					duration/1000);
		duration += intval(spec['tool_ms'] * this.getInstanceProp('time_multiplier'));
	}
...
function onToolUse(details){ // defined by crafty_tasker
	if (this.failed) return false;
	if (!details) return false;
...
function onVerify(pc){ // defined by crafty_tasker
	var can_create = true;
	var count = 0;

	var txt = '';

	if (this.make_spec['class_tsid']){
		txt = 'Would you like to make '+this.make_spec['count']+'x '+this.make_spec['class_tsid']+'?';
	}else{
		txt = 'I can\'t make that all right now, how about I get started by making some ';
		var make_list = utils.craftytasking_get_build_spec_list(this.make_spec);

		var counter = 0;
		for (var i in make_list){
			if (make_list[i]['make']){
				if (counter != 0) txt += ', ';
				txt += i+' ['+make_list[i]['count']+'x]';
				counter++;
			}
		}
		txt += '?';
	...
	if (can_create){
		txt += 'It will use '+this.make_spec['total_energy_cost']+' energy and take '+(this.make_spec['wait_ms'] * this.getInstanceProp('time_multiplier'))/1000+' seconds and reward you with '+this.make_spec['xp_reward']+'xp.';

		var make_list = utils.craftytasking_get_build_spec_list(this.make_spec, 0);
		if (num_keys(make_list) > 0){
			txt += '\r\rI will need to make the following:';
			count = 0;
			for (var i in make_list){
				if (make_list[i]['make']){
					if (count != 0) txt += ', ';
			...
			if (count == 0) txt+= ' nothing';

			txt += ' and use the ingredients:';
			count = 0;
			for (var i in make_list){
				if (!make_list[i]['make']){
					if (count != 0) txt += ', ';
