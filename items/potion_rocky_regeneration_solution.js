...
var label = "Rocky Regeneration Solution";
var version = "1355086256";
var name_single = "Rocky Regeneration Solution";
var name_plural = "Rocky Regeneration Solutions";
var article = "a";
var description = "A powerful stimulant for nubbin regeneration. NB: Does not work on rock roots, only on partially mined rock-remains.";
var is_hidden = false;
var has_info = true;
...
var base_cost = 0;
var input_for = [];
var parent_classes = ["potion_rocky_regeneration_solution", "potion_base", "tool_base", "takeable"];
...
var classProps = {
	...
	"points_capacity"	: "3",	// defined by tool_base (overridden by potion_rocky_regeneration_solution)
	"display_wear"	: "1",	// defined by tool_base
	"can_repair"	: "0",	// defined by tool_base
	"can_pour"	: "1",	// defined by potion_base (overridden by potion_rocky_regeneration_solution)
	"can_quaff"	: "0",	// defined by potion_base
	"pour_tooltip"	: "Regenerate a rock",	// defined by potion_base (overridden by potion_rocky_regeneration_solution)
	"quaff_tooltip"	: ""	// defined by potion_base
};

function initInstanceProps(){
	this.instanceProps = {};
	this.instanceProps.points_remaining = "3";	// defined by tool_base (overridden by potion_rocky_regeneration_solution)
...
function canPour(pc){ // defined by potion_rocky_regeneration_solution
	if (!pc.achievements_has('shiny_nugget_award')) return {ok: 0, error:'You must have the Shiny Nugget Award to pour this.'};

	var targets = this.getValidTargets(pc);
	if (!targets) return {error: "There are no mineable rocks around here."};

	var target = targets.pop();
	if (!target) return {error: "There are no mineable rocks around here."};

	if (target.getInstanceProp('chunks_remaining') >= 50) return {ok: 0, error:'This rock doesn\'t need regeneration'};

	return {ok: 1};
}

function getValidTargets(pc){ // defined by potion_rocky_regeneration_solution
	function isMineable(it){ return it.is_mineable && it.isUseable() ? true : false; }
	var target = pc.findCloseStack(isMineable, 200);
	if (!target) return [];
	return [target];
}

function onPour(pc, msg){ // defined by potion_rocky_regeneration_solution
	if (!msg.target) return {ok:0, error:'No mineable rock could be found.'};

	msg.target.setInstanceProp('chunks_remaining', 50);

	return {ok: 1, msg: 'The rock now looks as good as new!'};
}
...
function getAssetInfo(){
	return {
		...
		'thumb': "\nSha6fnl+AAAAAElFTkSuQmCC",
	};
};
...
var itemDef = {
	...
	asset_swf_v	: "\/c2.glitch.bz\/items\/2012-04\/potion_rocky_regeneration_solution-1334270050.swf",
	admin_props	: true,
	obey_physics	: true,
	in_background	: false,
	in_foreground	: false,
...
log.info("potion_rocky_regeneration_solution.js LOADED");
