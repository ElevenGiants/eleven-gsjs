...
var label = "High-Class Hoe";
var version = "1355086256";
var name_single = "High-Class Hoe";
var name_plural = "High-Class Hoes";
var article = "a";
var description = "More refined than your average hoe, the High-Class Hoe is a hoe you'll be proud to be seen with. Extra investment brings a five-times longer lifespan: With this glamorous new companion you'll be hoeing the whole night through.";
var is_hidden = false;
var has_info = true;
...
var base_cost = 600;
var input_for = [];
var parent_classes = ["high_class_hoe", "hoe", "tool_base", "takeable"];
...
var classProps = {
	...
	"points_capacity"	: "250",	// defined by tool_base (overridden by high_class_hoe)
...
function initInstanceProps(){
	this.instanceProps = {};
	this.instanceProps.points_remaining = "250";	// defined by tool_base (overridden by high_class_hoe)
...
verbs.aggressive_hoeing_off = { // defined by hoe
	"name"				: "aggressive hoeing off",
	"ok_states"			: ["in_pack"],
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Wear hoe down slower but lose the iMG bonus",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){

		if (pc.imagination_has_upgrade('hoeing_more_img_more_wear')){
			if (!pc.aggressive_off) { 
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		pc.aggressive_off = true;
		pc.sendActivity("You are now hoeing gently, wearing it down less but losing the iMG bonus.");
...
verbs.aggressive_hoeing_on = { // defined by hoe
	"name"				: "aggressive hoeing on",
	"ok_states"			: ["in_pack"],
	"requires_target_pc"		: false,
	"requires_target_item"		: false,
	"include_target_items_from_location"		: false,
	...
	"is_emote"			: false,
	"sort_on"			: 50,
	"tooltip"			: "Wear hoe down faster and gain more iMG",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		pc.aggressive_off = false;
		pc.sendActivity("You are now wearing the hoe down faster and gaining a bigger iMG bonus. Hoe, hoe, hoe!");
...
verbs.hoe = { // defined by hoe
	...
	"is_emote"			: false,
	"sort_on"			: 57,
	"tooltip"			: "Or, drag hoe to a weedy patch",
	"is_drop_target"		: false,
	"conditions"			: function(pc, drop_stack){
	...
	"handler"			: function(pc, msg, suppress_activity){
		...
		// find the nearest patch and invoke the hoe verb there.
		// this is merely a proxy...

		if (this.proxyVerb(pc, msg, 'patch', 100, 'tend')) return true;
		if (this.proxyVerb(pc, msg, 'patch_dark', 100, 'tend')) return true;

		pc.sendActivity("But there's nothing to hoe!");
...
function getDescExtras(pc){
	var out = [];
	if (pc && (!pc.skills_has("soil_appreciation_1"))) out.push([2, "You can use this, but it's much better to learn <a href=\"\/skills\/1\/\" glitch=\"skill|soil_appreciation_1\">Soil Appreciation<\/a> first."]);
...
function getAssetInfo(){
	return {
		'position': {"x":-36,"y":-21,"w":70,"h":21},
		'thumb': "\nX68\/AX5s19VQHRaBAAAAAElFTkSuQmCC",
	};
};
...
var itemDef = {
	...
	asset_swf_v	: "\/c2.glitch.bz\/items\/2012-05\/high_class_hoe-1338243643.swf",
	admin_props	: true,
	obey_physics	: true,
	in_background	: false,
	in_foreground	: false,
...
itemDef.keys_in_pack = {
	"r"	: "drop",
	"v"	: "aggressive_hoeing_off",
	"n"	: "aggressive_hoeing_on",
	"g"	: "give",
	"h"	: "hoe",
	"e"	: "repair"
};

log.info("high_class_hoe.js LOADED");
